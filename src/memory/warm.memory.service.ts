...
export class WarmMemoryService implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        try {
            ...
            if (!exists) {
                ...
                this.logger.log(`Successfully created collection: ${this.collectionName}`);
            } else {
                const info = await this.client.getCollection(this.collectionName);
                this.logger.log(`Warm memory collection details: dim=${JSON.stringify(info.config.params.vectors)}`);
            }
            ...
            this.logger.log(`Ensured payload indexes exist for 'accessLevel' and 'userId'`);
        } catch (error: unknown) {
    ...
    async search(query: string, userId: string): Promise<WarmResult[]> {
        ...
        try {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            const detail = (error as any)?.body?.message || (error as any)?.response?.data?.message || '';
            this.logger.error(`Warm memory search failed for query [${query.slice(0, 50)}...]: ${msg} ${detail}`);
            return []; // Fails open
        }
    }
