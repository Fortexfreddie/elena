...
export class OnboardingDetector {
  ...
  async check(telegramId: string): Promise<UserRecognitionState> {
    try {
      ...
      if (!user) {
        this.logger.log(
          `[ONBOARDING_TRACE] User ${telegramId} is entirely new to Elena.`,
        );
        return 'unknown';
      }

      this.logger.log(
        `[ONBOARDING_TRACE] User ${telegramId} status from DB: ${user.onboardingStatus}`,
      );
      ...
      this.logger.error(
        `Failed to check user recognition for ${telegramId}`,
        error,
      );
