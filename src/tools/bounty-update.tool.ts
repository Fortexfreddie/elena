...
export class BountyUpdateTool implements AgentTool {
    ...
    name = 'bounty_update';
    description = 'Manage project bounties and tasks. Supports creating, updating, and listing open bounties.';
    ...
    async execute(args: Record<string, unknown>, context: AgentContext): Promise<ToolResult> {
        ...
        try {
            ...
            if (!internalUserId) return { success: false, error: 'User not found in system.' };
            ...
            switch (action) {
                case 'create': {
                    ...
                    return { success: true, data: { message: 'Bounty created successfully.', bountyId: bounty.id } };
                }
                case 'update': {
                    const bountyId = args['bountyId'] as string;
                    if (!bountyId) return { success: false, error: 'BountyId is required for update.' };
                    ...
                    return { success: true, data: { message: 'Bounty updated successfully.' } };
                }
                case 'list': {
                    const bounties = await this.prisma.bounty.findMany({
                        where: { status: { in: [BountyStatus.open, BountyStatus.in_progress] } },
