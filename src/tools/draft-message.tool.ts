...
export class DraftMessageTool implements AgentTool {
  ...
  name = 'draft_message';
  description =
    'Draft a complex message or announcement based on context. This tool does not send the message, it only returns the draft for the agent to use in its final response.';
      ...
      parameters: {
        ...
        properties: {
          draftFor: {
            type: Type.STRING,
            description:
              'What the draft is for (e.g., "announcement", "reply to founder", "code summary").',
          },
          keyPoints: {
            type: Type.ARRAY,
