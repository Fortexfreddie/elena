...
@Injectable()
export class ReminderDeliveryHandler {
  ...
  async handle(job: Job): Promise<void> {
    ...
    if (reminder.sent) {
      this.logger.warn(
        `[REMINDER] Reminder ${reminderId} already sent — skipping`,
      );
      return;
    }
    ...
    try {
      if (reminder.targetType === 'dm' && reminder.targetUserId) {
        ...
        this.logger.log(
          `[REMINDER] Sent DM reminder to ${reminder.targetUserId}`,
        );
      } else {
        ...
        this.logger.log(
          `[REMINDER] Sent group reminder to chat ${reminder.chatId}`,
        );
      }
      ...
      this.logger.log(
        `[REMINDER] Reminder ${reminderId} delivered and marked sent`,
      );
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `[REMINDER] Failed to deliver reminder ${reminderId}: ${msg}`,
      );
