...
export class HitlProcessor extends WorkerHost {
  ...
  async process(job: Job<HITLResumeJob>): Promise<void> {
    ...
    try {
      ...
      if (!isClaimed) {
        this.logger.warn(
          `[EXECUTION_TRACE] HITL job ${jobId} already claimed or processed.`,
        );
        return;
      }
      ...
      if (!raw) {
        ...
        if (!chatId) {
          this.logger.error(
            `Invalid jobId format during expiry handling: ${jobId}`,
          );
          return;
        }
        await this.replySender.sendReply(
          chatId,
          '⚠️ This confirmation request has expired (5m limit).',
        );
        return;
      }
      ...
      try {
        ...
        this.logger.error(
          `HITL payload corrupted for jobId ${jobId} — cannot parse JSON`,
        );
        if (chatId !== 'unknown') {
      ...
      if (
        ...
        this.logger.error(
          `HITL payload has unexpected shape for jobId ${jobId}`,
        );
        if (chatId !== 'unknown') {
          await this.replySender
            .sendReply(
      ...
      context.decryptedSecretsSet = new Set(
        context.decryptedSecretsArray ?? [],
      );
      ...
      if (result.success && result.data && typeof result.data === 'object' && 'message' in result.data) {
        ...
        // Fallback for tools that don't return a "message" field but have data
